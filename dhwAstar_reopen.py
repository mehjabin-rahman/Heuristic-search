...
def calculate_weighted_hueristic(start, goal, weight):
    ...
    return heuristic
def calculate_weighted_dhhueristic(start, goal, weight):
    #print(type(start))
    heuristic=0
    #heuristic = (abs(start[0]-goal[0])+abs(start[1]-goal[1]))*weight
    i=randint(0,100)
    j=randint(0,3)
    if j==1:
        heuristic=heuristicDH4[0][i]
    if j==2:
        heuristic=heuristicDH4[0][i]
    if j==3:
        heuristic=heuristicDH3[0][i]
    if j==0:
        heuristic=heuristicDH4[0][i]
...
def dhwastarr(start, goal, map,t,hueristic_weight=1, movement_cost=1):
    node_expanded=0
    cost_so_far=[]
    #-----------
    dhlist_status=[]
    ...
    if t==1:
        ...
        start_node = Node(start, calculate_weighted_dhhueristic(start, goal, hueristic_weight))
    goal_node = Node(goal, 0, None, -1)
    open_list = Open()
    closed_list = {}
    ...
    while(len(open_list)>0):
        ...
        for neighbor in neighbors:
            ...
            if t==1:
                neighbor_node = Node(neighbor, calculate_weighted_hueristic(neighbor, goal, hueristic_weight),
                                     current_node,
                                     current_node.g_cost + movement_cost)
            else:
                neighbor_node = Node(neighbor, calculate_weighted_dhhueristic(neighbor, goal,hueristic_weight), current_node,
                                 current_node.g_cost + movement_cost)
