...
def calculate_zero_heuristic(start, goal):
    #heuristic = (abs(start[0]-goal[0])+abs(start[1]-goal[1]))
    return 0
# wh cost
def calculate_weighted_heuristic(start, goal, weight):
    #print(type(start))
...
def wastarrin(start, goal, map,heuristic_weight=1, movement_cost=1):
    node_expanded=0
    cost_so_far=[]
    #-----------
    dhlist_status=[]
    ...
    start_node = Node(start, calculate_weighted_heuristic(start, goal,heuristic_weight))
    goal_node = Node(goal, 0, None, -1)
    open_list = Open()
    closed_list = {}
    ...
    while(len(open_list)>0):
        ...
        for neighbor in neighbors:
            ...
            if p==0:
                neighbor_node = Node(neighbor, calculate_zero_heuristic(neighbor, goal),
                                     current_node,
                                     current_node.g_cost + movement_cost)
            else:
                neighbor_node = Node(neighbor, calculate_weighted_heuristic(neighbor, goal,heuristic_weight), current_node,
                                 current_node.g_cost + movement_cost)
            ...
            if (output[0]):
                ...
                if((neighbor_node < node)[0]):
                    node.update(current_node, neighbor_node.g_cost, (heuristic_weight*neighbor_node.h_cost))
