...
def calculate_hueristic(start, goal, movement_cost):
    heuristic = (abs(start[0]-goal[0])+abs(start[1]-goal[1]))*movement_cost
    return heuristic

def calculate_zero_hueristic(start, goal, movement_cost):
    #heuristic = (abs(start[0]-goal[0])+abs(start[1]-goal[1]))*movement_cost
    return 0
'''
def landmark_heuristic(a, z):
    ...
    return h
'''
def calculate_weighted_hueristic(start, goal, weight, movement_cost):
...
def anytime_weighted_astar(start, goal, map, hueristic_weight, movement_cost=1):

    start_node = Node(start, calculate_hueristic(start, goal, movement_cost), h_cost2=calculate_weighted_hueristic(start, goal, hueristic_weight, movement_cost))
    ...
    while(len(open_list)>0):
        try:
            ...
            for neighbor in neighbors:
                ...
                neighbor_node = Node(neighbor, calculate_hueristic(neighbor, goal, movement_cost), current_node, current_node.g_cost+movement_cost, calculate_weighted_hueristic(neighbor, goal, hueristic_weight, movement_cost))
